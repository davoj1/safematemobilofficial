...
interface SafemateTake5ReviewPageProps {
  onBack?: () => void
  onSubmit?: () => void
  formData?: {
    taskToday: string
...
const mockHazardsAndControls = [
  { 
    hazard: "Slippery surfaces due to recent rain", 
    control: "Use non-slip footwear and install warning signs",
    implemented: 'yes' as const,
    comment: "All signs installed and team briefed"
  },
  { 
    hazard: "Overhead power lines in work area", 
    control: "Maintain safe distance and use spotter",
    implemented: 'no' as const,
    comment: "Need to coordinate with electrical team for isolation"
  }
]

// SafeMate Take 5 exposure questions
...
const SafemateTake5ReviewPage: React.FC<SafemateTake5ReviewPageProps> = ({
  onBack,
  onSubmit,
  formData
}) => {
  ...
  // Calculate status based on exposures
  const getFormStatus = () => {
    // Count the number of "NO" answers in exposures
    const noAnswers = Object.values(formData?.exposureAnswers || {}).filter(answer => answer === 'no').length
    
    // Good: All answers are YES (0 NO answers)
    if (noAnswers === 0) {
      return {
        type: 'good',
  ...
  const getImplementedStyling = (implemented: string) => {
    switch (implemented) {
      case "yes":
        return "bg-[#ecfdf3] border-[#17b26a] text-[#344054]"
      case "no":
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-3">
            <div className="flex justify-between items-center">
              ...
              <span className="text-[#344054] text-sm font-medium leading-5">#ST5001</span>
            </div>
            ...
            <div className="bg-white rounded-xl border border-[#d0d5dd] p-3">
              <div className="flex flex-col gap-3">
                <div className="flex justify-between items-center">
                  <h3 className="text-[#266273] text-base font-bold leading-6">Mine Company Info</h3>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-9 h-9 rounded-md flex items-center justify-center">
                        <img src={safemateLogo} alt="SafeMate" className="w-full h-6" />
                      </div>
                  ...
                  {/* Custom Site */}
                  <div className="flex flex-col gap-1">
                    <div className="text-[#344054] text-sm font-medium leading-5">Custom Site</div>
                    {formData?.selectedSite ? (
                      <div className="text-[#101828] text-base font-semibold leading-6">
                        {formData.selectedSite.name}
                      </div>
                    ) : (
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="text-[#667085] text-sm font-normal leading-5">
                      {formData?.taskToday || "Install safety barriers around the excavation site and ensure proper signage is in place."}
                    </div>
                  </div>

                  {/* Why should I work safe */}
                  <div className="flex flex-col gap-1">
                    <div className="text-[#344054] text-sm font-medium leading-5">Why should I work safe?</div>
                    <div className="text-[#667085] text-sm font-normal leading-5">
                      {formData?.whyWorkSafe || "Because safety means I can keep doing the job I love. And to make sure no one on my team gets hurt today."}
                    </div>
                  </div>
                      ...
                      <div className="flex gap-2">
                        {formData.photos.map((photo, index) => (
                          <div key={index} className="relative w-20 h-20 rounded-xl overflow-hidden">
                            <img src={URL.createObjectURL(photo)} alt={`Work site photo ${index + 1}`} className="w-full h-full object-cover" />
            ...
            <div className="bg-white rounded-xl border border-[#d0d5dd] p-3">
              <div className="flex flex-col gap-3">
                <div className="flex flex-col gap-0.5">
                  ...
                  <div className="text-[#667085] text-sm font-medium leading-5">
                    {Object.keys(formData?.exposureAnswers || {}).length}/6 ({Math.round((Object.keys(formData?.exposureAnswers || {}).length / 6) * 100)}%)
                  </div>
                </div>
                
                <div className="flex flex-col gap-0 pl-1.5">
                  {exposureQuestions.map((question, index) => {
                    // Map the question to the form data keys
                    const questionKeys = [
                      'trainedAndCompetent',
                      'workWillNotImpactOthers',
                      'correctToolsAndEquipment',
                      'identifiedLineOfFire',
                      'canCompleteWithoutClimbing',
                      'commitToPausingIfTaskChanges'
                    ]
                    
                    const questionKey = questionKeys[index]
                    const answer = formData?.exposureAnswers?.[questionKey] || 'na'
                    const comment = formData?.exposureComments?.[`${questionKey}Comment`] || ''
                    const photo = formData?.exposurePhotos?.[`${questionKey}Photo`]
                    ...
                    return (
                      <div key={index} className="flex flex-col gap-2 py-2 border-b border-[#eaecf0] last:border-b-0">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <div className={`px-3 py-2 rounded-lg border-[1.2px] w-20 flex items-center justify-center ${getAnswerStyling(answer.toUpperCase())}`}>
                            <span className="text-xs font-semibold leading-[18px]">{answer.toUpperCase()}</span>
                          </div>
                        </div>
                        
                        {/* Comment and Photo Section - Only show for NO answers */}
                        {answer === 'no' && (comment || photo) && (
            ...
            <div className="bg-white rounded-xl border border-[#d0d5dd] p-3">
              <div className="flex flex-col gap-2.5">
                ...
                <div className="flex flex-col gap-4">
                  {(formData?.hazardsAndControls && formData.hazardsAndControls.length > 0 ? formData.hazardsAndControls : mockHazardsAndControls).map((hazardItem, index) => (
                    <div key={index} className="flex flex-col gap-2">
                      ...
                      {/* Implemented Status */}
                      <div className="flex justify-between items-center">
                        <div className="text-[#344054] text-sm font-medium leading-5">Implemented</div>
                        <div className={`px-3 py-2 rounded-lg border-[1.2px] w-20 flex items-center justify-center ${getImplementedStyling(hazardItem.implemented || 'na')}`}>
                          <span className="text-xs font-semibold leading-[18px] uppercase">{hazardItem.implemented || 'N/A'}</span>
                        </div>
                      </div>

                      {/* Comment if not implemented */}
                      {hazardItem.implemented === 'no' && hazardItem.comment && (
                        <div className="flex flex-col gap-1">
                          <div className="text-[#344054] text-sm font-medium leading-5">Comment</div>
                          <div className="text-[#667085] text-sm font-normal leading-5">
                            {hazardItem.comment}
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-[#101828] text-sm font-medium leading-5">
                        {formData?.otherCompany || "SafeMate"}
                      </span>
                    </div>
                  </div>
...
export default SafemateTake5ReviewPage
