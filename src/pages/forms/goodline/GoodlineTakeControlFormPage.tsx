...
interface GoodlineTakeControlFormPageProps {
  selectedSite?: {
    id: string
    name: string
    location: string
  ...
  onBack?: () => void
  onNext?: (formData: GoodlineTakeControlFormData) => void
}

interface GoodlineTakeControlFormData {
  taskToday: string
  howWillTakeControl: string
  photos: File[]
  goodlineSpecificField: string
}

const GoodlineTakeControlFormPage: React.FC<GoodlineTakeControlFormPageProps> = ({
  selectedSite,
  onBack,
  onNext,
}) => {
  const [formData, setFormData] = useState<GoodlineTakeControlFormData>({
    taskToday: '',
    howWillTakeControl: '',
    photos: [],
    goodlineSpecificField: '',
  })
  ...
  const handleGoodlineSpecificChange = (value: string) => {
    setFormData(prev => ({ ...prev, goodlineSpecificField: value }))
  }
  ...
  const handleNext = () => {
    console.log('Goodline Take Control form data:', formData)
    onNext?.(formData)
  }
  ...
  const isFormValid = formData.taskToday.trim() !== '' && 
                     ...
                     formData.goodlineSpecificField.trim() !== ''

  return (
    <div className="h-screen flex flex-col bg-[#f8f7f2] overflow-hidden">
      {/* Header - Fixed */}
      <HeaderWithBack
        title="Goodline Take Control"
        progress="1/4"
        onBack={handleBack}
      ...
      <div className="flex-1 px-5 py-3 space-y-4 overflow-y-auto">
        ...
        {/* Goodline Specific Field */}
        <div className="space-y-2">
          <h2 className="font-bold text-[#266273] text-lg leading-7 text-center">
            Goodline Safety Requirements
          </h2>
          <TextareaInput
            placeholder="Describe any Goodline-specific safety requirements..."
            value={formData.goodlineSpecificField}
            onChange={handleGoodlineSpecificChange}
            rows={3}
          />
        </div>
...
export default GoodlineTakeControlFormPage
