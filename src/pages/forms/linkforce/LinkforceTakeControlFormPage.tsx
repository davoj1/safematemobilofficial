...
interface LinkforceTakeControlFormPageProps {
  selectedSite?: {
    id: string
    name: string
    location: string
  ...
  onBack?: () => void
  onNext?: (formData: LinkforceTakeControlFormData) => void
}

interface LinkforceTakeControlFormData {
  taskToday: string
  howWillTakeControl: string
  photos: File[]
  linkforceSpecificField: string
}

const LinkforceTakeControlFormPage: React.FC<LinkforceTakeControlFormPageProps> = ({
  selectedSite,
  onBack,
  onNext,
}) => {
  const [formData, setFormData] = useState<LinkforceTakeControlFormData>({
    taskToday: '',
    howWillTakeControl: '',
    photos: [],
    linkforceSpecificField: '',
  })
  ...
  const handleLinkforceSpecificChange = (value: string) => {
    setFormData(prev => ({ ...prev, linkforceSpecificField: value }))
  }
  ...
  const handleNext = () => {
    console.log('Linkforce Take Control form data:', formData)
    onNext?.(formData)
  }
  ...
  const isFormValid = formData.taskToday.trim() !== '' && 
                     ...
                     formData.linkforceSpecificField.trim() !== ''

  return (
    <div className="h-screen flex flex-col bg-[#f8f7f2] overflow-hidden">
      {/* Header - Fixed */}
      <HeaderWithBack
        title="Linkforce Take Control"
        progress="1/4"
        onBack={handleBack}
      ...
      <div className="flex-1 px-5 py-3 space-y-4 overflow-y-auto">
        ...
        {/* Linkforce Specific Field */}
        <div className="space-y-2">
          <h2 className="font-bold text-[#266273] text-lg leading-7 text-center">
            Linkforce Safety Protocol
          </h2>
          <TextareaInput
            placeholder="Describe any Linkforce-specific safety protocols..."
            value={formData.linkforceSpecificField}
            onChange={handleLinkforceSpecificChange}
            rows={3}
          />
        </div>
...
export default LinkforceTakeControlFormPage
