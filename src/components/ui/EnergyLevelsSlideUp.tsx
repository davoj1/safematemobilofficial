...
interface EnergyLevelsSlideUpProps {
  isOpen: boolean
  onClose: () => void
  teamMembers: TeamMemberOption[]
  onSubmit: (payload: { memberIds: string[], message: string }) => void
}

const EnergyLevelsSlideUp: React.FC<EnergyLevelsSlideUpProps> = ({
  isOpen,
  onClose,
  teamMembers,
  onSubmit
  ...
  const handleSubmit = () => {
    ...
    const message = "🔋 Energy check - how are your levels?"
    onSubmit({ memberIds: selectedMemberIds, message })
    resetAndClose()
  }
    ...
    <div className="fixed inset-0 z-50 flex items-end">
      ...
      <div className="relative bg-white rounded-t-[20px] w-full h-[85vh] flex flex-col pt-12 pb-6 px-4 shadow-xl">        
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-[#667085] mt-0.5">Select team members to check energy levels</p>
        </div>
        ...
        <div className="px-5 py-4 border-t border-[#eaecf0] flex gap-3">
          ...
          >
            Check Energy ({selectedMemberIds.length})
          </Button>
        </div>
      </div>
    </div>
...
export default EnergyLevelsSlideUp
